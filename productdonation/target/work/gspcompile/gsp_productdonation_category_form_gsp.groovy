...
printHtmlPart(2)
invokeTag('textField','g',10,['name':("name"),'maxlength':("50"),'required':(""),'value':(categoryInstance?.name),'class':("form-control")],-1)
printHtmlPart(3)
...
printHtmlPart(2)
invokeTag('datePicker','g',19,['name':("createdDate"),'precision':("day"),'value':(categoryInstance?.createdDate),'class':("form-control")],-1)
printHtmlPart(5)
expressionOut.print(hasErrors(bean: categoryInstance, field: 'status', 'error'))
printHtmlPart(6)
invokeTag('message','g',25,['code':("category.status.label"),'default':("Status")],-1)
printHtmlPart(7)
invokeTag('checkBox','g',28,['name':("status"),'value':(categoryInstance?.status)],-1)
printHtmlPart(8)
}
...
public static final long LAST_MODIFIED = 1431850709563L
